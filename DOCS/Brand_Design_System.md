...
| **Reversed** (White on Light) | Use only when logo sits on a coloured accent element (e.g. gold `#D4AF37` chip) |
...
--color-gold:     #D4AF37;   /* Zari Gold — rules, CTA underlines, cursor dot */
...
.closing-cta-btn  { color: #20222D; border-bottom-color: #D4AF37; }
```
...
| `#D4AF37` on `#FAF0E6` | Gold on Cream | ~3.2:1 | ⚠️ Decorative only — not body text |

> **Rule:** Gold (`#D4AF37`) is used only for decorative rules, underlines, and the cursor. Never use gold as body text colour — it does not meet AA contrast on light backgrounds.
...
/* Indian floral outlines — gold (on cream surfaces) */
opacity: 0.05;
...
```css
/* Gold accent rule (beneath eyebrows, between sections) */
.accent-rule {
  width: 28px;
  height: 1px;
  background: #D4AF37;
...
- **Outer ring** — 32px, `border: 1px solid rgba(212,175,55,0.5)`, transitions to pointer position with 80ms lag
- **Inner dot** — 6px, `background: #D4AF37`, snaps to cursor position
...
Foreground illustrations in warm gold (#D4AF37), warm ivory (#FFF8F0), and
...
Scattered gold Indian floral outlines at 5% opacity.
