/**
 * Download Drive (or other remote) service_cards.image_url values,
 * convert to WebP under public/uploads/services/, and UPDATE Neon.
 * Skips same-origin paths (/service_section/…, /uploads/…).
 *
 * Usage: npm run migrate:service-images
 */
...
const outDir = path.join(root, 'public', 'uploads', 'services')
...
function isLocalPath(imageUrl) {
  ...
  try {
    const u = new URL(t)
    if (u.pathname.startsWith('/service_section/')) return true
...
async function downloadBytes(imageUrl) {
  ...
  throw new Error(`Unsupported image_url: ${trimmed.slice(0, 80)}`)
}
...
const rows = await sql`
  SELECT id, image_url FROM service_cards ORDER BY id ASC
`
...
console.log(`[migrate-service-images] ${rows.length} card(s)`)
...
for (const row of rows) {
  ...
  if (!imageUrl || isLocalPath(imageUrl)) {
    skipped += 1
    console.log(`  skip  ${id}  ${imageUrl || '(empty)'}`)
    continue
  }

  const publicPath = `/uploads/services/${safeFileStem(id)}.webp`
  const diskPath = path.join(outDir, `${safeFileStem(id)}.webp`)

  try {
    console.log(`  migrate ${id} …`)
    const raw = await downloadBytes(imageUrl)
    const webp = await toWebp(raw)
    writeFileSync(diskPath, webp)
    await sql`
      UPDATE service_cards
      SET image_url = ${publicPath}, updated_at = NOW()
      WHERE id = ${id}
    `
    migrated += 1
...
console.log(
  `[migrate-service-images] done — migrated=${migrated} skipped=${skipped} failed=${failed}`,
)
